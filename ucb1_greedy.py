import numpy as np
from ucb_learner import *
from Environment import *
from Greedy_optimizer import *


class ucb1_greedy(ucb_learner):
    def __init__(self, daily_users, n_products, n_arms, prices, env: Environment, sw=np.inf):
        super().__init__(n_products, n_arms)
        self.daily_users = daily_users
        self.means = np.zeros((n_products, n_arms))
        self.widths = np.ones((n_products, n_arms)) * np.inf
        self.prices = prices
        self.env = env
        self.greedy_opt = Greedy_optimizer(self.env)
        self.sw = sw

    def pull_arms(self):
        sampled_cr = np.minimum(np.array([self.means + self.widths]), 1) # limit to 1 for all the crs
        arms_pulled = self.greedy_opt.run(conversion_rates=sampled_cr)["combination"]
        return arms_pulled

    def update(self, arms_pulled, cr_data):
        super().update(arms_pulled, cr_data)
        for product_idx in range(self.n_products):
            self.means[product_idx, arms_pulled[product_idx]] = np.sum(np.multiply(np.array(self.rewards_per_arm[product_idx][arms_pulled[product_idx]])[:, 0], np.array(self.rewards_per_arm[product_idx][arms_pulled[product_idx]])[:, 1]))/(np.sum(np.array(self.rewards_per_arm[product_idx][arms_pulled[product_idx]])[:, 1]))
        for product_idx in range(self.n_products):
            for arm_idx in range(self.n_arms):
                if len(self.pulled) < self.sw:
                    n = np.sum(np.multiply(np.array(self.pulled, dtype=np.int32)[:, 0, product_idx] == arm_idx, np.array(self.pulled, dtype=np.int32)[:, 1, product_idx]))
                else:
                    n = np.sum(np.multiply(np.array(self.pulled[-self.sw:-1], dtype=np.int32)[:, 0, product_idx] == arm_idx, np.array(self.pulled[-self.sw:-1], dtype=np.int32)[:, 1, product_idx]))
                if n > 0:
                    self.widths[product_idx, arm_idx] = np.sqrt(2 * np.log(self.t) / n)
                else:
                    self.widths[product_idx, arm_idx] = np.inf
