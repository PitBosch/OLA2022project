...
##praticamente se non ho capito male dovrei creare 5 dict-> 1 per ogni prodotto --> No in realtà devi creare un dizionario con 5 elementi Prodotto: posizione del prezzo
    def execute(self, user, links, lambda_prob, products):
        ...
        if page_kind != 5:
            user.probabilities[:, page_kind] = 0
            #selected_price=self.products[page_kind].prices[self.products[page_kind].label]
            selected_price = self.products[page_kind].get_selected_price()
    ...
    def simulate_day(self, users_number, users_probs, links, lambda_prob): #TODO USER PROBS LINKS E LAMBDA DA TOGLIERE
        ...
        for i in range(users_number):
            ...
            daily_profit += self.execute(self.users[user_kind], links, lambda_prob, self.products) #userprofit
