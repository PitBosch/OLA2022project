...
class Environment:
    ...
    def expected_reward111(self, price_combination, conversion_rates=None, alpha_ratios=None, n_prod=None, graph_weights=None):
        """Method that compute the expected reward related to the price combination passed to the function.
            If the only argument passed is the price combination the function returns the theoretical expected
            reward. The method can receive 4 optional arguments:
                - conversion rate: list of matrices representing the probabilities to buy a product i at a price j for the user k
                - alpha_ratios : list of lists of probabilities to land on product i at first for user j
                - n_prod : list of average number of product sold for a user
                - graph_weights : list of matrices describing the probabilities of a click on a
    ...
    def single_reward(self, user_index, price_combination):
        """ Method to compute the expected reward for a single user category given a specific price combination for the 5 products """
    ...
    def product_reward(self, primary: Product, primary_history: list[Product], q_link: list[float], link: list[Product], price_combination, user_index):
        ...
        if s_1 in primary_history and s_2 in primary_history:
            return b_i * exp_margin + q_link.copy()[-1] * self.product_reward(link.copy()[-1], primary_history, q_link.copy()[:-1], link.copy()[:-1], price_combination, user_index)
        # all exceptions have been treated, let's now compute the expected reward in the basic case
