...
class Environment:
    ...
    #Teoricamente questo potrebbe essere un metodo di usercat?
    def execute(self, user, price_combination):
        ...
        page_index = user.start_event()  # è il draw dalla random choice per capire su che pagina attero tenendo in considerazione gli alpha
        if page_index != 5:
            #user.probabilities[:, page_index] = 0
        ...
        return 0 #lo devo mettere perchè altrimenti restituisco NoneType quando page_index=5
            ...
            #selected_price=self.products[page_index].prices[self.products[page_index].label]
            #I'm saving only the index of the first product select, I should save the whole combination of price.
            #pageindex is also a product index


            #webpage it's useless now
            #wp = WebPage(self.products[page_index], user)
    ...
    def simulate_day(self, users_number, users_probs, price_combination): #TODO USER PROBS DA TOGLIERE
        ...
        for i in range(users_number):
            ...
            up=self.execute(self.users[user_kind], price_combination) #userprofit
    ...
    def user_profit(self, user, price_combination, product_index):
        ...
        first_secondary = self.products[self.Secondary_dict.get(self.products[product_index].name)[0]]  # TODO: i collegamenti tra i diversi oggetti sono fissi o tipici per classe di utente?
        ...
        if second_click and second_secondary not in user.visited_products:
            ...
            # aggiungo il visited al tutto
        return margin
                # new_WebPage = WebPage(first_secondary, self.user)
                # new_WebPage.interact(self.links, self.products, lambda_prob, price_index)
            # clicko sul secondo e non l'ho ancora visitato

                #return margin+self.user_profit(user, price_combination[product_index], first_secondary.label)
                # second_new_WebPage = WebPage(second_secondary, self.user)
                # second_new_WebPage.interact(self.links, self.products, lambda_prob, price_index)
    # ma price_index non viene mai aggiornato e non veniva mai aggiornato?
