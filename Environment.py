...
class Environment:
    ...
    def buy(self, user_index, prod_ind, price_ind): 
        return np.random.binomial(1, self.theoretical_values['conversion_rates'][user_index][prod_ind, price_ind])

    def user_simulation(self, user_index, price_combination, product_index, to_save_dict: dict):
        ...
        primary_bought = self.buy(user_index, product_index, price_ind)
        ...
        if first_click:
            ...
            self.user_simulation(user_index, price_combination, first_secondary.index, to_save_dict)
        ...
        if second_click:
            ...
            self.user_simulation(user_index, price_combination, second_secondary.index, to_save_dict)
        return


    def execute(self, user_index, price_combination, to_save_dict: dict):
        ...
        self.user_simulation(user_index, price_combination, page_index, to_save_dict)
        return
    ...
    def simulate_day_context(self, daily_users, price_combination_list, context_matrix, to_save: list):
        ...
        for i in range(n_users):
            ...
            self.execute(user_ind, price_combination_list[comb_ind], to_save_data[feat_key])
        ...
        for data_dict in list(to_save_data.values()):
            ...
            if "alpha_ratios" in to_save:
                data_dict["alpha_ratios"] = data_dict["initial_prod"]/np.sum(data_dict["initial_prod"]+1e-6)
    ...
    def simulate_day(self, daily_users, price_combination, to_save: list, aggregated = True):
        ...
        for i in range(n_users):
            ...
            # we increment the daily profit of the website by the profit done with the simulated user
            self.execute(user_kind, price_combination, to_save_data[user_kind])
            # notice that we have passed only the dictionary for the specific user category sampled
