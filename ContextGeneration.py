...
class ContextGeneration():
    ...
    def run(self, simulinfo, feat_prob_mat):
        ...
        generate_feat_prob_matrix(feat_prob_mat) #NON HO CAPITO STA LINEA
        self.bertot=Greedy_optimizer.run(conversion_rates=simulinfo["conversion_rates"], alpha_ratios=simulinfo["alpha_ratios"], n_prod=simulinfo["n_prod"], group_list=group_list[0], feat_prob_mat=feat_prob_mat)['expected_reward'] #DEPTH DIVENTA LIST
    ...
    def split0(self, simulinfo, feat_prob_mat):
        ...
        #matrice dei ber forse utile eh
        cv1=ber00+ber01 
        ...
        return [0,0,feature_splitted] #restituisco su quale feature ho splittato e il nuovo bertot, sulla matrice ho già fatto l'update


    def split1(self, simulinfo, feat_prob_mat, spfeature_list,branch, ber):
        ...
        gr=list(filter(lambda x: len(x) == 1, group_list))
        #get only the groups that have length one (this works only for the second step not for the third)
        #possible solution:
        gr=list(filter(lambda x: x[abs(1-spfeature_list[0])]==spfeature_list[0])) #da riguardare perchè non ha senso spfeature_list
        ...
        #devo gestire meglio sta roba, nel senso: devo fare in modo che i gruppi siano sempre coerenti, ora non lo sono e sto considerando un caso dove ho solo 2 gruppi
        cv1=ber0+ber1 #The context value of the first feature is computed as the sum of the other two
        cv2=0 #the context value of the secodn feature is zero
        feature_splitted=self.compare(cv1,cv2,ber)
        self
        if feature_splitted!=-1:
